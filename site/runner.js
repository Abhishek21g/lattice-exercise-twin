(function () {
  ...
  const TOTAL = config.totalHours;
  const FAIL = config.failureHour;
  ...
  function renderMesh(container, h) {
    ...
    container.innerHTML = `
      <svg viewBox="0 0 480 240" width="100%" height="240">
        ...
        <circle cx="90" cy="120" r="40" fill="#16161a" stroke="${stroke(true)}" stroke-width="2"/>
        <circle cx="90" cy="120" r="10" fill="${color(true)}"/>
        <text x="90" y="185" text-anchor="middle" fill="#8b909a" font-size="12">Sensor tower</text>
        <circle cx="240" cy="120" r="40" fill="#16161a" stroke="${stroke(relayOk)}" stroke-width="2"/>
        <circle cx="240" cy="120" r="10" fill="${color(relayOk)}"/>
        <text x="240" y="185" text-anchor="middle" fill="#8b909a" font-size="12">Relay-3</text>
        <circle cx="390" cy="120" r="40" fill="#16161a" stroke="${stroke(relayOk)}" stroke-width="2"/>
  ...
  function drawMiniChart(canvas, h) {
    ...
    for (let i = 0; i <= TOTAL; i++) {
      ...
      const y = pad.t + plotH - (taskAt(i) / 1.2) * plotH;
  ...
  function updateReceipt(h) {
    ...
    if (FAIL !== null && h >= FAIL) {
      ...
      if (detail) detail.textContent = "At T+" + h + "h: relay offline — Investigate task cannot assign.";
    } else if (FAIL === null) {
      box.className = "receipt-box pass";
      if (verdict) { verdict.className = "verdict-big pass"; verdict.textContent = "DEPLOY"; }
      if (detail) detail.textContent = "At T+" + h + "h: mesh healthy — tasks still assigning.";
    } else {
